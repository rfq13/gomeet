...
export class LiveKitClient {
   ...
   * Enable camera (video)
   */
  async enableCamera(): Promise<Track | null> {
    if (!this.room) {
      throw new Error("Not connected to a room");
    }

    try {
      const tracks = await this.room.localParticipant.enableCamera();
  ...
  async disableCamera(): Promise<void> {
    ...
    try {
      await this.room.localParticipant.disableCamera();
   ...
   */
  async enableMicrophone(): Promise<Track | null> {
    if (!this.room) {
      throw new Error("Not connected to a room");
    }

    try {
      const tracks = await this.room.localParticipant.enableMicrophone();
  ...
  async disableMicrophone(): Promise<void> {
    ...
    try {
      await this.room.localParticipant.disableMicrophone();
  ...
  getParticipantByIdentity(identity: string): RemoteParticipant | null {
    ...
    return this.room.participants.get(identity) || null;
  }
  ...
  getAllParticipants(): Participant[] {
    ...
    if (this.room) {
      participants.push(...Array.from(this.room.participants.values()));
    }
  ...
  getParticipantInfo(participant: Participant): ParticipantInfo {
    ...
    return {
      ...
      isCameraEnabled: tracks.some(
        (track) => track.kind === "video" && !track.isMuted
      ),
      isMicrophoneEnabled: tracks.some(
        (track) => track.kind === "audio" && !track.isMuted
      ),
      tracks,
    };
  }
...
export type { Room, RemoteParticipant, LocalParticipant, Track };
